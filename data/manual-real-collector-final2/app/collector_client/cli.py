...
from app.collector_client.telemetry import collect_sanitized_facts
...
def _run_once(config: CollectorConfig, heartbeat_reason: str = "run_once_completed") -> CommandResult:
    ...
    save_state(config.state_path, state)
    try:
        uploaded = _upload_pending(config, state["outbox"], heartbeat_reason)
    ...
    source_keys = [str(item.get("source_refs", {}).get("source_key", "")) for item in facts]
    source_keys = [source_key for source_key in source_keys if source_key and not source_key.startswith(("health:", "source-status:"))]
    if source_keys:
        state["cursor"]["last_source_key"] = max(source_keys)
...
def _upload_pending(config: CollectorConfig, outbox: list[dict[str, object]], heartbeat_reason: str) -> int:
    if not outbox:
        return 0
...
def _run_pending_diagnostic(config: CollectorConfig) -> int:
    ...
    if command.get("command_id") != "collect_codex_error_context":
        ...
        result = {
            "status": "succeeded",
            "summary": "客户端完成 Codex 错误上下文白名单补证，已生成脱敏结构化结果。",
            "projection": {
...
def _get_json(server_url: str, path: str) -> dict[str, Any]:
    with urllib.request.urlopen(_url(server_url, path), timeout=5) as response:
