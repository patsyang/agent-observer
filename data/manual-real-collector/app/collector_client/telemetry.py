...
def _record_fact(collector_id: str, sequence: int, source_key: str, path: Path, line_number: int, record: dict) -> dict | None:
    event_type = _clean(record.get("type") or record.get("event_type") or record.get("kind") or "unknown")
...
def _error_fact(common: dict, record: dict) -> dict:
    tool = _clean(record.get("tool") or record.get("command") or "unknown_tool")
    phase = _clean(record.get("phase") or record.get("status") or "unknown_phase")
    exit_code = int(record.get("exit_code") or record.get("code") or 1)
    signature = f"codex_error:{tool}:{phase}:{exit_code}:{_hash(str(record.get('summary', '')) )[:8]}"
    return {
        **common,
        "fact_type": "error",
...
def _usage_fact(common: dict, record: dict) -> dict:
    units = int(record.get("total_tokens") or record.get("tokens") or record.get("units") or 0)
    tags = record.get("activity_tags") or record.get("activity_tag") or ["unknown"]
    ...
    session_id = _ref(record.get("session_id") or record.get("session") or "unknown")
    conversation_id = _ref(record.get("conversation_id") or record.get("conversation") or "unknown")
    return {
        **common,
        "fact_type": "usage",
        "category": "usage",
        ...
        "projection": {"units": units, "activity_tag": activity_tag, "tag_count": len(tags) if isinstance(tags, list) else 1},
        "usage": {
            "scope": "session",
            "units": units,
...
def _risk_fact(common: dict, record: dict) -> dict | None:
    operation = _clean(record.get("operation") or record.get("action") or record.get("tool") or "")
    path = _clean(record.get("path") or record.get("target") or "")
    if operation not in HIGH_RISK_OPERATIONS and "delete" not in path and "auth" not in path.lower():
        return None
    ...
    return {
        **common,
        ...
        "quality": "high",
        "severity": "medium",
        "summary": f"检测到高风险本地操作类别：{operation or 'workspace_change'}，对象类型 {object_type}。",
        "projection": {"operation": operation or "workspace_change", "object_type": object_type, "path_hash": _hash(path)[:16]},
...
def _low_evidence_fact(common: dict, record: dict) -> dict:
    return {
        **common,
        ...
        "projection": {"observed_keys": sorted(_safe_key(key) for key in record.keys())[:12]},
    }
...
def _source_refs(collector_id: str, sequence: int, source_key: str, path: Path, line_number: int, record: dict) -> dict:
    return {
        ...
        "line": line_number,
        "conversation_ref": _ref(record.get("conversation_id") or record.get("conversation") or source_key),
        "session_ref": _ref(record.get("session_id") or record.get("session") or path.stem),
    }


def _is_error(record: dict) -> bool:
    status = str(record.get("status") or record.get("level") or "").lower()
...
def _is_usage(record: dict) -> bool:
    return any(key in record for key in ("total_tokens", "tokens", "units", "activity_tag", "activity_tags"))
