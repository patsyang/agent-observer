...
def command_category(command: str) -> str:
    ...
    if re.search(r"\b(rm|del|erase|rmdir)\b(?!\-)|remove-item|git\s+reset\s+--hard|git\s+clean\b", lowered):
        return "destructive"
    if re.search(r"\b(chmod|chown|icacls|takeown|cacls|xcacls)\b|set-acl\b|attrib\b.*[+\-][rRhHsSaAiI]", lowered):
    ...
    if any(term in lowered for term in ("npm run build", "vite build")) or re.search(r"\btsc\b", lowered):
        return "build"
    if "get-content" in lowered or re.search(r"\bcat\s", lowered):
        return "file_read"
    if re.search(r"\btype\s", lowered):
        return "file_read"
...
def safe_key(value: object) -> str:
    ...
    if sensitive_categories_from_text(lowered):
