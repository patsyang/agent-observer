...
from app.sensitivity import SENSITIVE_MARKERS, sensitive_categories_from_text
...
def _codex_facts(
    *,
    ...
    last_source_key = str(cursor.get("last_source_key", ""))
    recent_seen = set(str(item) for item in cursor.get("recent_source_keys", []))
    facts: list[dict] = []
    seen_source_keys: set[str] = set()
    recent_budget = max(0, min(max(1, max_events), 100))
    for source_key, path, line_number, record in _recent_tail_records(sessions_dir, cutoff):
        if source_key in recent_seen or source_key in seen_source_keys:
            continue
        ...
        if len(facts) >= recent_budget:
            break
    for source_key, path, line_number, record in _records(sessions_dir, cutoff):
        if len(facts) >= max(1, max_events):
            break
        if source_key <= last_source_key or source_key in seen_source_keys:
            continue
        fact = _record_fact(collector_id, sequence, source_key, path, line_number, record, upload_raw)
        if fact:
            fact["source_specific"]["priority_stream"] = "history_cursor"
            facts.append(fact)
            seen_source_keys.add(source_key)
    return facts
...
def _recent_tail_records(sessions_dir: Path, cutoff: datetime) -> Iterable[tuple[str, Path, int, dict]]:
    paths = []
    ...
    for path in sessions_dir.rglob("*"):
        ...
        try:
            ...
            continue
        modified_at = datetime.fromtimestamp(mtime, UTC)
        if modified_at >= cutoff and modified_at >= live_cutoff:
            paths.append((mtime, path))
    paths.sort(key=lambda item: (item[0], item[1].as_posix()), reverse=True)
    for _, path in paths[:5]:
        try:
            ...
            continue
        records = list(_parse_records(text))
        for line_number, record in reversed(records[-250:]):
            source_key = f"{path.as_posix()}:{line_number:08d}"
            yield source_key, path, line_number, record
...
def _sensitive_fact(common: dict, record: dict) -> dict:
    categories = sorted(_sensitive_categories(record))
    ...
    return {
        **common,
        ...
        "quality": "high",
        "severity": "high",
        "summary": f"Codex 会话触达敏感线索：{', '.join(categories) if categories else 'unknown'}。",
        "projection": {"object_type": object_type, "category_count": len(categories), "sensitive_categories": categories},
...
def _sensitive_categories(record: dict) -> set[str]:
    ...
    for key in record:
        values.update(sensitive_categories_from_text(key))
    for value in (args.get("command"), args.get("path"), args.get("workdir"), payload.get("name")):
        values.update(sensitive_categories_from_text(value))
    return values
